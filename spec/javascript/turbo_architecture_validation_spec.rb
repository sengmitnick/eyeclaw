  ...
  describe 'ActionCable Broadcast Type Validation' do
    it 'validates that broadcast types match frontend handlers' do
      ...
      source_files.each do |source_file|
        ...
        # Read file lines to check for disable comments
        lines = content.split("\n")
        ...
        broadcasts.each do |broadcast|
          ...
          # Check up to 2 lines before the broadcast for stimulus-validator: disable-next-line comment
          should_skip = false
          [line_number - 1, line_number - 2].each do |check_line|
            if check_line > 0 && check_line <= lines.length
              line_content = lines[check_line - 1]
              if line_content && line_content.strip.start_with?('#') && line_content.include?('stimulus-validator: disable-next-line')
                should_skip = true
                break
              end
            end
